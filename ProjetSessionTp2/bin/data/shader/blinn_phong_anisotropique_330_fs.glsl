...
uniform float anisotropique_factor;
...
void main()
{
  ...
  if (reflection_diffuse > 0.0)
  {
    ...
    // brightness anisotropique (prend en compte l'angle de la caméra et modifie le facteur en conséquence)
    float anisotropique_brightness = brightness * pow((1.0 - n.z), anisotropique_factor);

    // calculer le niveau de réflexion spéculaire (n • h)
    reflection_specular = pow(max(dot(n, h), 0.0), anisotropique_brightness);
  }
  ...
  fragment_color = vec4(
    ...
    pow((1.0 - abs(abs(n.z) - abs(v.z))), anisotropique_factor) * color_specular * reflection_specular, 1.0);
}
